...
//   const { data, loading, error , method } = useApi(
...
export default function User() {
  ...
  const { data: updateResponse, loading: loadingPut, error: errorPut } = useApi(
    putData?.url,
    putData?.token,
    "PUT",
    putData?.body
  ...
  if (error || errorPut) return <p>Erreur : {error || errorPut}</p>;
